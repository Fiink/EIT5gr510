...
%             dPhase = mean(dPhase);
%         end
    X = ['Faseforskel: ',num2str(mean(dPhase))];
        disp(X);    
    if mean(dPhase) > pi
            disp('#### FASE OVER PI');
    ...
    thetaNy = 28.648*dPhase;

    %Check for imaginary parts, indiciating an error during logging
    %, else add to ret
    if imag(thetaGammel) ~= 0
        error = error + 1;
        disp('FANDT FEJL!!!!!!!!!!!!!!!');
    else
        ret = ret + thetaGammel;
        retNy = retNy + thetaNy;
        X = ['thetaGammel for iteration ',num2str(i),': ',num2str(thetaGammel)];
        disp(X)
        X = ['thetaNy for iteration ',num2str(i),': ',num2str(thetaNy)];
        disp(X)
    end
end
...
disp(X);
X = ['Endelig gammel vinkel: ',num2str(ret)];
disp(X);
X = ['Endelig   ny   vinkel: ',num2str(retNy)];
disp(X);
